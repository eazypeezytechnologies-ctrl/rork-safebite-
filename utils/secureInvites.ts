...
export async function createSecureFamilyInvite(
  ...
  try {
    ...
    if (memberError) {
      console.error('[SecureInvites] Member count check failed:', memberError.message);
    }
    ...
    const currentCount = (members?.length || 0) + (pendingInvites?.length || 0);
    ...
    if (error) {
      console.error('[SecureInvites] Insert failed:', error.message);
      logAuditEvent({
      ...
      return { token: '', invitation: null, error: error.message };
    }
    ...
    return { token: '', invitation: null, error: msg };
  }
}
...
export async function getSecureFamilyInvites(
  ...
  try {
    ...
    if (error) {
      console.error('[SecureInvites] Fetch error:', error.message);
      return [];
    }
